...
double rayCubeIntersection(const vec3& P0, const vec3& V0, const mat4& inverse, vec4 &objspacenormal)
{
	...
	if(near >= 0) {
		objspacenormal = glm::vec4(0.f, 0.f, 0.f, 0.f);
		objspacenormal[near] = lesserplane ? -1.f : 1.f;
	}
